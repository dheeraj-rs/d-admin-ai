...
export default function Sidebar() {
    ...
    return (
        <>
            <div
                className={`bg-[var(--d-admin-surface-ground)] backdrop-blur-3xl border-r border-[var(--border-main)] h-full transition-all duration-300 ease-in-out relative flex flex-col flex-shrink-0 ${isOpen ? 'w-64' : 'w-16'}`}
            >
                <div className="flex items-center h-[60px] relative w-full shrink-0">
                    ...
                    <div className="w-16 h-full flex items-center justify-center shrink-0 z-10">
                        <button
                            ...
                            className={`w-10 h-10 rounded-full bg-[var(--d-admin-surface-section)] text-[var(--d-admin-text-color)] flex items-center justify-center transition-all duration-300 shadow-lg border border-[var(--border-main)] hover:scale-105 active:scale-90 group cursor-pointer z-20 backdrop-blur-md`}
                    ...
                    {isOpen && (
                        <div className="flex items-center gap-2 overflow-hidden animate-in fade-in slide-in-from-left-2 duration-300">
                              <span className="text-[11px] font-bold text-[var(--d-admin-text-color-secondary)] tracking-widest uppercase">Navigation</span>
                        </div>
                    )}
                </div>
                            ...
                            <div key={idx} className="relative w-full h-11 group">
                                ...
                                >
                                    ...
                                    <div className="absolute left-0 w-16 h-full flex items-center justify-center shrink-0 z-10">
                                        <div className={`flex items-center justify-center rounded-lg transition-all ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-[var(--d-admin-text-color-secondary)] group-hover:text-[var(--d-admin-text-color)]'}`}>
                ...
                <div className={`flex-1 overflow-y-auto overflow-x-hidden mt-6 transition-opacity duration-300 ${isOpen ? 'opacity-100 delay-100' : 'opacity-0 pointer-events-none delay-0'}`}>
                    <div className="px-5 py-2 text-[10px] font-black text-[var(--d-admin-text-color-secondary)] uppercase tracking-[0.2em]">Recent active</div>
                    <div className="px-3 space-y-1 w-[240px]">
                        {RECENT_CHATS.map((chat, idx) => (
                            <div 
                                 ...
                                 className="group/chat flex items-center gap-2 p-2 px-3 rounded-xl hover:bg-[var(--d-admin-surface-hover)] cursor-pointer text-[13px] transition-all truncate border border-transparent hover:border-[var(--border-main)]"
                            >
                                <div className="w-1 h-1 rounded-full bg-indigo-500 opacity-0 group-hover/chat:opacity-100 transition-opacity" />
                                <span className="truncate font-medium text-[var(--d-admin-text-color)]">{chat}</span>
                            </div>
                        ))}
                    </div>
                </div>
