...
export default function AIBuildPanel({ inputValue, setInputValue, handleSubmit, setViewState }: AIBuildPanelProps) {
    return (
        <div className="w-full flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-3 duration-400">
            {/* Input box */}
            <div className="relative flex items-center bg-[#2a2a2a] rounded-[28px] p-2 pl-3 pr-3 shadow-xl w-full group focus-within:ring-2 focus-within:ring-blue-500/40 transition-all border border-white/[0.05]">
                <PlusActionMenu />
                <input
                    id="ai-build-input"
                    type="text"
                ...
                <div className="flex items-center gap-1.5 pr-0.5">
                    <button
                        id="ai-build-mic-btn"
                        className="text-gray-400 hover:text-gray-200 w-[36px] h-[36px] flex items-center justify-center rounded-full transition-colors"
                    >
