...
export default function EditorTopBar() {
    ...
    const { activeTab, isShareOpen } = state;
    const { setViewState, setActiveTab, setIsShareOpen } = actions;
        ...
        <div className="flex items-center justify-between px-2.5 sm:px-4 h-14 bg-[#0B0B0D] border-b border-white/5 select-none z-50">
            ...
            <div className="flex items-center gap-2 sm:gap-4 flex-none sm:flex-1 min-w-0 sm:pr-4">
                <button 
                    ...
                    className="flex items-center justify-center w-10 h-10 sm:w-auto sm:h-10 sm:px-4 hover:text-white text-gray-400 rounded-lg transition-colors shrink-0 max-w-full min-w-0 group cursor-pointer"
                    title="Hide Editor"
                >
